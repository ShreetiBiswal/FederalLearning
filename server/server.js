...
io.on("connection", (socket) => {
  ...
  socket.on("join_as_hospital", () => {
    if (!connectedHospitals.includes(socket.id)) {
      ...
      console.log(
        `[🏥] Hospital Registered: ${socket.id} (${connectedHospitals.length}/${TARGET_HOSPITALS})`,
      );
    }
  ...
  socket.on("upload_weights", (payload) => {
    if (!connectedHospitals.includes(socket.id)) {
      console.log(
        `[⚠️] Ignoring weights from unauthorized/evaluator socket: ${socket.id}`,
      );
      return;
    }
    ...
    if (!payload || !payload.algo || typeof payload.dataset_size !== "number") {
      console.error(
        `[!] ERROR: Malformed payload received from ${socket.id}. Rejecting.`,
      );
    ...
    if (payload.is_compressed) {
      ...
      try {
        ...
        socket.emit("server_error", {
          message: "Failed to decompress weights.",
        });
        return;
      }
    }
    ...
    const logAlgoName = isSmoteDisabled
      ? `${payload.algo}_nosmote`
      : payload.algo;

    console.log(
      `[⬇️] Received ${logAlgoName.toUpperCase()} weights from ${socket.id} (Size: ${payload.dataset_size})`,
    );
    ...
    if (receivedUpdates.length === TARGET_HOSPITALS) {
      ...
      let totalDataSize = receivedUpdates.reduce(
      ...
      console.log(
        `   📈 Local Average Accuracy: ${(globalAcc * 100).toFixed(2)}% | Loss: ${globalLoss.toFixed(4)}`,
      );

      // --- Log to CSV using the dynamic logAlgo ---
      fs.appendFileSync(
        CSV_FILE,
        `${currentLogAlgo},${currentRound},${globalAcc},${globalLoss}\n`,
      );
      ...
      try {
        ...
        if (
          currentAlgo === "fedavg" ||
          currentAlgo === "ce_fedavg" ||
          currentAlgo === "wsm_ce_fedavg"
        ) {
        ...
        if (currentRound <= MAX_ROUNDS) {
          console.log(
            `[⬆️] Broadcasting Round ${currentRound} master weights...`,
          );
          io.emit("apply_global_update", {
            global_weights: globalModelUpdates,
            global_c: globalControlVariateState,
            round: currentRound,
          });
        } else {
          ...
          const FINAL_MODEL_PATH = path.join(
            MODELS_DIR,
            `final_master_model_${currentLogAlgo}.json`,
          );

          fs.writeFileSync(
            FINAL_MODEL_PATH,
            JSON.stringify(globalModelUpdates),
          );
          ...
          setTimeout(() => {
            console.log("🛑 Shutting down server for next experiment...");
            process.exit(0);
          }, 3000);
        }
      } catch (error) {
        ...
        io.emit("server_error", {
          message: "Aggregation failed. Round aborted.",
        });
      }
    }
  });
  ...
  socket.on("disconnect", () => {
    ...
    if (isTrainingActive) {
      console.log(
        `[!] WARNING: Hospital dropped during active training. Waiting for reconnection to resume Round ${currentRound}.`,
      );
      receivedUpdates = receivedUpdates.filter(
        (update) => update.id !== socket.id,
      );
    }
  });
});
...
server.listen(PORT, () => {
  ...
  console.log(
    `Waiting for ${TARGET_HOSPITALS} skewed hospital nodes to connect...`,
  );
});
