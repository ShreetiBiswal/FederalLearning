...
export function aggregateSCAFFOLD(updates) {
    ...
    if (updates[0].local_c) {
        ...
        let c_weight = 1.0 / updates.length; // Simple arithmetic mean for control variates

        c_layers.forEach(layer => {
            let baseC = deepScale(updates[0].local_c[layer], c_weight);
            for (let k = 1; k < updates.length; k++) {
                let scaledC = deepScale(updates[k].local_c[layer], c_weight);
