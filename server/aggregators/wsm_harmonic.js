...
export function aggregateErrorAwareHarmonic(updates) {
    ...
    for (let i = 0; i < rawScores.length; i++) {
        ...
        console.log(`   [📊] Hosp: ${s.id.substring(0,4)} | Vol: ${(s.n_fedavg*100).toFixed(1)}% | Class: ${(s.n_class*100).toFixed(1)}% | Err: ${(s.error_signal*100).toFixed(1)}% | 🏆 FINAL WSM: ${(finalWeight * 100).toFixed(2)}%`);
